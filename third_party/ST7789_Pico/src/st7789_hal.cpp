...
bool HAL::writeDataDma(const uint16_t* data, size_t len) {
    ...
    while (remaining > 0) {
        ...
        for (size_t i = 0; i < transfer_size; i++) {
            _dma_buffer[i] = rgb565_to_dma_word(src_ptr[i]);
        }
...
bool HAL::writeDataDmaSolidColor(uint16_t color, size_t pixel_count) {
    if (pixel_count == 0) {
        return true;
    }

    const uint16_t swapped = rgb565_to_dma_word(color);
    ...
    while (remaining > 0) {
        size_t transfer_size =
            (remaining > _dma_buffer_size / 2) ? _dma_buffer_size / 2 : remaining;
        for (size_t i = 0; i < transfer_size; ++i) {
            _dma_buffer[i] = swapped;
        }

        _dma_busy = true;
        dma_channel_set_read_addr(_dma_tx_channel, _dma_buffer, false);
        dma_channel_set_trans_count(_dma_tx_channel, transfer_size, true);
