...
struct Config {
    ...
    /** Added to CASET (column) addresses — many 240x320 modules need 0; some need 52/34 etc. */
    uint16_t col_offset;
    /** Added to RASET (row) addresses */
    uint16_t row_offset;
    /**
     * If non-zero, column/row addresses after offset are clamped to
     * [col_offset, col_offset + ram_col_max - 1] and same for rows (panel GRAM size).
     * If zero, no clamp (addresses still uint16_t).
     */
